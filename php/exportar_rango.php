...
$consulta_dos = $_POST['fecha_final'];
$seleccion = $_POST['seleccion'];

// print_r($consulta);
// exit();

$date_dos = new DateTime($consulta_dos);
$bien_dos = $date_dos->format('d/m/Y');
...
$sql = "SELECT e.nomina, e.name, e.apellido_Paterno, e.apellido_Materno, R.fecha_hora, t.id_turno, t.turno, t.entrada, t.salida, c.descripcion FROM empleado e INNER JOIN registro R ON e.nomina = R.nomina INNER JOIN turno t ON t.id_turno = e.id_turno_F INNER JOIN company c ON c.id_company = e.id_company_F WHERE STR_TO_DATE(R.fecha_hora, '%d/%m/%Y') >= STR_TO_DATE('".$bien."', '%d/%m/%Y') AND STR_TO_DATE(R.fecha_hora, '%d/%m/%Y') <= STR_TO_DATE('".$bien_dos."', '%d/%m/%Y') AND c.descripcion = '".$seleccion."';";

// SELECT e.nomina, e.name, e.apellido_Paterno, e.apellido_Materno, R.fecha_hora, t.id_turno, t.turno, t.entrada, t.salida, c.descripcion FROM empleado e INNER JOIN registro R ON e.nomina = R.nomina 
// INNER JOIN turno t ON t.id_turno = e.id_turno_F 
// INNER JOIN company c ON c.id_company = e.id_company_F
// WHERE STR_TO_DATE(R.fecha_hora, '%d/%m/%Y') >= STR_TO_DATE('01/07/2022', '%d/%m/%Y') AND STR_TO_DATE(R.fecha_hora, '%d/%m/%Y') <= STR_TO_DATE('02/07/2022', '%d/%m/%Y') AND c.descripcion = 'movistar';
