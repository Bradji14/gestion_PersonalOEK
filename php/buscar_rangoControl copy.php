...
if (!empty($_POST['consulta']) && !empty($_POST['consultados']) && !empty($_POST['seleccion'])) 
{
  ...
  if($seleccion=='Movistar'){
    
  
  
  // $sql="SELECT p.nomina,c.descripcion, fecha_reg,e.name, CONCAT(entrada,count(p.nomina)) as Entradas, CONCAT(salida,count(p.nomina)) as Salidas FROM pase p
  // INNER JOIN empleado e ON p.nomina =e.nomina
  // INNER JOIN company c ON e.id_company_F =c.id_company
  // WHERE c.descripcion = 'Movistar' AND p.fecha_reg BETWEEN '$consulta' and '$consulta_dos'
  // GROUP BY p.nomina ORDER BY p.nomina";
  ...
  <div id="div2" class="mb-3" style="height:200px; overflow-y:scroll;">
     ...
     while ($fila = $query->fetch(PDO::FETCH_ASSOC)) 
    {
        ...
        <tr>   
            ...
            <td class='text-center'>". $fila['salida']."        "."</td>
