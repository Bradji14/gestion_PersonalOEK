...
$polo= date("Y-m-d",strtotime($fecha_ini."+ 1 day")); 
$polo2= date("Y-m-d",strtotime($polo."+ 5 day")); 
        ...
        if ($numeroDeFilas5 > 0) {
            ...
            if($numeroDeFilas > 0){
                ...
                if ($esquema == 2) {
                    ...
                    $query2->execute([$nomina,$dateHour, 'A',$datenow,$ip_add,1]);

                    for($i = $polo; $i < $polo2; $i++){
                        
                        $sql15 = "SELECT * from pase WHERE nomina='$nomina' and fecha_reg LIKE '%$i%' and status=2";
                        ...
                        if($numeroDeFilas5 > 0){
                            ...
                            $sql2 = "INSERT INTO pase(nomina,fecha_entrada,entrada,fecha_reg,ipRed,status) VALUES (?,?,?,?,?,?)";    
                            $query2 = $conn->prepare($sql2);
                            $query2->execute([$nomina,'-', 'F',$i,$ip_add,2]);
                        }
                   } 
                    $_SESSION['nomina']=$nomina;
                    $_SESSION['ac'] = "Asistencia correcta";
                ...
                elseif (intval(str_replace(":", '', substr($date, -6))) <= intval(str_replace(":", '', $entrada)) + 5) {
                    ...
                    if($numeroDeFilas > 0){
                        ...
                        $sql2 = "INSERT INTO pase(nomina,fecha_entrada,entrada,fecha_reg,ipRed,status) VALUES (?,?,?,?,?,?)";    
                        $query2 = $conn->prepare($sql2);
                        $query2->execute([$nomina,$dateHour, 'R',$datenow,$ip_add,1]);
                ...
                if($numeroDeFilas > 0){
                    ...
                    $sql2 = "INSERT INTO pase(nomina,fecha_entrada,entrada,fecha_reg,ipRed,status) VALUES (?,?,?,?,?,?)";    
                    $query2 = $conn->prepare($sql2);
                    $query2->execute([$nomina,$dateHour, 'FR',$datenow,$ip_add,1]);
