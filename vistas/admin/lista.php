...
$stmt ="SELECT p.nomina,p.entrada,p.salida,p.status,p.fecha_reg,p.fecha_entrada,p.fecha_salida,
p.hr_break,p.hr_break_fin,e.name,e.apellido_Paterno,t.turno,q.descripcion as esqui,
e.apellido_Materno,c.descripcion
FROM pase p
INNER JOIN empleado e ON p.nomina = e.nomina
INNER JOIN company c ON e.id_company_F =c.id_company
INNER JOIN esquema q ON e.id_esquema_f =q.id_esquema
INNER JOIN turno t ON e.id_turno_F =t.id_turno
WHERE p.status=1 || p.status=2 order by p.fecha_reg";
