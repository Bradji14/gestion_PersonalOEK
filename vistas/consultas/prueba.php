...
if(isset($_POST['buscar'])){
    ...
    $valores=array();
    $valores['existe']="0";
    // SELECT *, company.descripcion FROM empleado  INNER JOIN turno ON empleado.id_turno_F=turno.id_turno INNER JOIN company ON empleado.id_company_F=company.id_company WHERE empleado.nomina ='$id' ;
    ...
    while ($row = $query3->fetch()) {
        $valores['existe']="1";
        $valores['name']=$row['name'].' '.$row['apellido_Paterno'];
        $valores['turno_F']=$row['turno'].' '.$row['entrada'].'-'.$row['salida'];
        $valores['compa']=$row['descripcion'];
        $valores['id_turno_F']=$row['id_turno_F'];
        sleep(1);
