  ...
  // $_SESSION['error']="campos vacios";
    ...
    && !empty($numbers) && !empty($options))
    {
      ...
      if($numeroDeFilas<=0){
        ...
        header("Location: http://192.168.100.200:50/asistencias_empleados/vistas/sistemas/permisos.php");


      }else{
            $sql = "INSERT INTO permisos(id_nomina,id_area,id_jefe,motivo,tiempo_solicitado,fechaPermiso,company,created_at) VALUES (?,?,?,?,?,?,?,?)";
            $query = $conn->prepare($sql);
            $query->execute([$empleado,$arae,$jefe,$motivo,($numbers.' '.$options),$newDate,$compa,$fechaActual]);
