...
include_once "../../bd/conexion.php";
    $sql2= "SELECT  e.name, p.id_jefe, e.nomina,j.nombre,j.apellidos,j.id, e.apellido_Paterno,e.apellido_Materno,p.company,p.id_permiso, p.fechaPermiso, p.motivo,p.quest, p.tiempo_solicitado, c.descripcion from empleado e  INNER JOIN permisos p ON e.nomina = p.id_nomina INNER JOIN company c ON e.id_company_F=c.id_company INNER JOIN jefes_empresa j ON p.id_jefe= j.id WHERE p.estatus=0 AND p.company= 'movistar'";
    $query2 = $conn->prepare($sql2);
    $query2->execute();
    $numeroDeFilas = $query2->rowCount();

    $sql3= "SELECT  e.name, p.id_jefe, e.nomina,j.nombre,j.apellidos,j.id, e.apellido_Paterno,e.apellido_Materno,p.company,p.id_permiso, p.fechaPermiso, p.motivo,p.quest,p.tiempo_solicitado, c.descripcion from empleado e  INNER JOIN permisos p ON e.nomina = p.id_nomina INNER JOIN company c ON e.id_company_F=c.id_company INNER JOIN jefes_empresa j ON p.id_jefe= j.id WHERE p.estatus=1 AND p.company = 'movistar'";
    $query3 = $conn->prepare($sql3);
    $query3->execute();
    $numeroDeFilas3 = $query3->rowCount();

    $sql1= "SELECT  e.name, p.id_jefe, e.nomina,j.nombre,j.apellidos,j.id, e.apellido_Paterno,e.apellido_Materno,p.company,p.id_permiso, p.fechaPermiso, p.motivo,p.quest, p.tiempo_solicitado, c.descripcion from empleado e  INNER JOIN permisos p ON e.nomina = p.id_nomina INNER JOIN company c ON e.id_company_F=c.id_company INNER JOIN jefes_empresa j ON p.id_jefe= j.id WHERE p.estatus=2 AND p.company= 'movistar'";
