...
$query ="SELECT  e.id_empleado, e.nomina, e.name, e.apellido_Paterno, e.apellido_Materno,p.status,p.nomina,p.entrada,p.salida,p.fecha_reg,p.fecha_salida,p.fecha_entrada, c.descripcion FROM empleado e 
INNER JOIN pase p ON e.nomina = p.nomina
INNER JOIN company c ON c.id_company = e.id_company_F
WHERE c.descripcion = 'Movistar' and p.status=1 and fecha_reg LIKE '%$date%'";
...
if(isset($_POST["search"]["value"]))
{
 ...
 $query .= '
  and e.nomina LIKE "%'.$_POST["search"]["value"].'%"';
}
...
{
    $query .= 'and fecha_reg LIKE "%'.$date.'%"  AND c.descripcion = "Movistar" ORDER BY e.nomina ';
}
$query1 = '';
...
$query ="SELECT  e.id_empleado, e.nomina, e.name, e.apellido_Paterno, e.apellido_Materno,p.status,p.nomina,p.entrada,p.salida, t.entrada AS nominar, t.id_turno, t.turno, t.entrada, t.salida, c.descripcion FROM empleado e 
INNER JOIN pase p ON e.nomina = p.nomina
