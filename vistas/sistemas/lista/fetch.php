...
SELECT  e.id_empleado, e.nomina, e.name, e.apellido_Paterno, e.apellido_Materno,p.nomina,p.entrada,p.salida,p.fecha_reg,p.fecha_salida,p.fecha_entrada, c.descripcion FROM empleado e 
INNER JOIN pase p ON e.nomina = p.nomina 
INNER JOIN turno t ON t.id_turno = e.id_turno_F 
INNER JOIN company c ON c.id_company = e.id_company_F 
AND e.nomina = '1026'";
...
{
 $query .= 'and fecha_reg LIKE "%'.$date.'%" AND c.descripcion = "Administrativo" ORDER BY e.nomina ';
}
$query1 = '';
